...
export const DataStructureBox: React.FC<DataStructureBoxProps> = ({ 
  ...
  const dictItems = !isArrayOrTuple ? Object.entries(items as Record<string, string | number>) : [];
  ...
  const badgeTextClass = isTuple ? 'text-cyan-300' : 'text-purple-300';
  const gridBorderClass = isTuple ? 'border-cyan-500/30' : 'border-purple-500/30';
  ...
  return (
    ...
    >
      <span className={`text-[10px] font-black uppercase tracking-widest ${badgeTextClass} font-mono leading-none px-1`}>
        {name} {isTuple ? '(TUPLE)' : variant === 'array' ? '[LIST]' : '{DICT}'}
      </span>
      ...
      {isArrayOrTuple ? (
        ...
        <div className={`flex flex-col border ${gridBorderClass} rounded-lg overflow-hidden bg-slate-950/40`}>
          {dictItems.map(([key, val], idx) => {
            ...
            return (
              ...
              >
                <div className={`flex-1 p-2 border-r ${gridBorderClass} transition-colors duration-300 ${
